...
describe("Attaching a document to a workflowitem.", function () {
  ...
  const uploadDocument = (fileName) => {
    // open edit dialog:
    cy.get("div[title=Edit] > button").should("be.visible").click();
  ...
  it("A document can be validated.", function () {
    ...
    cy.wait("@validate").get(`button[label="Validated!"] > span`).should("contain", "Identical");
  });
  ...
  it("Validation of wrong document fails.", function () {
    ...
    cy.wait("@validate").get(`button[label="Changed!"] > span`).should("contain", "Different");
  });
  ...
  it("The filename and document name are shown correctly", function () {
    ...
    // Check document name
    cy.get("[data-test=workflowitemDocumentFileName]").should("be.visible").contains(fileName);
