describe("`Cookie management`", function() {
  ...
  it("If a user is logged in, the login state must be true in the redux state", async () => {
    cy.window()
      .its("store")
      .invoke("getState")
      .then(s => {
        expect(s.getIn(["login", "isUserLoggedIn"])).to.be.true;
      });
  });

  it("If a user is logged out, the login state must be false in the redux state", async () => {
    cy.get("#logoutbutton")
      .should("be.visible")
      .click()
      .then(() => {
        cy.window()
          ...
          .invoke("getState")
          .then(s => {
            expect(s.getIn(["login", "isUserLoggedIn"])).to.be.false;
          });
      });
  });

  it("In case of failed login attempt, the login state must be false in the redux state", async () => {
    cy.get("#logoutbutton")
      .should("be.visible")
      .click()
      .then(() => {
  ...
  it("If a user is logged in, the JWT token is not stored in the redux state", async () => {
    cy.window()
      .its("store")
      .invoke("getState")
      .then(s => {
        expect(s.getIn(["login", "jwt"])).to.be.undefined;
      });
  });

  it("If a user is logged out, the JWT token is not stored in the redux state", async () => {
    cy.get("#logoutbutton")
      .should("be.visible")
      .click()
      .then(() => {
  ...
  it("In case of failed login attempt, JWT token is not stored in redux state", async () => {
    cy.get("#logoutbutton")
      .should("be.visible")
      .click()
      .then(() => {
