...
const queryProvisionState = async (axios) =>
  await withRetry(() => axios.get("/provisioned").then((res) => res.data.data));

const setProvisionStartFlag = async (axios) => {
  await withRetry(() => axios.post("/provisioning.start", {}));
};

const setProvisionEndFlag = async (axios) => {
  await withRetry(() => axios.post("/provisioning.end", {}));
};
