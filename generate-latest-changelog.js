...
const run = async () => {
  ...
  if (!version) {
    console.error(
      "Version not defined. Run command as `npm run generate-changelog 2.9.0`"
    );
  ...
  const newReleaseChangelog = template
    .replace(
      "<!-- ## [x.x.x] - YYYY-MM-DD -->",
      `
## [${version}] - ${today.toISOString().split("T")[0]}`
    )
    .replace(
      "<!-- ### Added -->",
      `### Added
...
${changelogText}`
  ...
  ${newReleaseChangelog}`
    ...
    .replace(
      /\[unreleased\]: https:\/\/github\.com\/openkfw\/TruBudget\/compare\/v(\d+\.\d+\.\d+)\.\.\.main[\r\n]\[(\d+\.\d+\.\d+)\]/m,
