...
class RoleSelectionContent extends Component {
  ...
  createChips = selectedRoles =>
    selectedRoles.map((role, index) => {
      return (
        <Chip key={index} style={styles.chip} onRequestDelete={() => this.onRemoveChip(role)}>
          {role.displayName}
        </Chip>
      );
    });
  ...
  captureEnterClick = event => {
    if (event.charCode === 13) {
      ...
      const index = unselectedDataSource.findIndex(
        item => item.role.toLowerCase() === _.trim(this.state.searchText).toLowerCase()
      );
      if (index > -1) {
    ...
    return (
      <div style={styles.container}>
        <AutoComplete
          ...
          menuCloseDelay={0}
        />
        <div style={styles.chipWrapper}>{this.createChips(this.props.selections)}</div>
      </div>
    );
  };
}
