...
/**
 * Creates the swagger schema for the `/workflowitem.validateHash` endpoint
 *
...
function mkSwaggerSchema(server: AugmentedFastifyInstance): Object {
  return {
    ...
    schema: {
      description: "Compares a given hash against a hash of a document",
      tags: ["workflowitem"],
...
/**
 * Creates an http handler that handles incoming http requests for the `/workflowitem.validateHash` route
 *
...
export function addHttpHandler(
  ...
  server.register(async function () {
    server.post(
      `${urlPrefix}/workflowitem.validateHash`,
      ...
      (request, reply) => {
        ...
        service
          .matches(expectedSHA256, ctx, user, projectId, subprojectId, workflowitemId, documentId)
          .then((validateWorkflowitemResult) => {
            if (Result.isErr(validateWorkflowitemResult)) {
              throw new VError(validateWorkflowitemResult, "workflowitem.validateHash failed");
            }
