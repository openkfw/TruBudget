...
export default class StorageServiceClient implements StorageServiceClientI {
  ...
  public async uploadObject(
    ...
    if (config.encryptionKey) {
      requestData.fileName = encrypt(config.encryptionKey, requestData.fileName);
      requestData.content = encrypt(config.encryptionKey, requestData.content);
    }
  ...
  public async downloadObject(id: string, secret: string): Promise<Result.Type<StorageObject>> {
    ...
    if (config.encryptionKey) {
      const fileName = decrypt(config.encryptionKey, documentObject.fileName);
      const base64 = decrypt(config.encryptionKey, documentObject.base64);
