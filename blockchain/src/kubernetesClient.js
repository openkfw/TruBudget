...
class KubernetesClient {
  ...
  async getServiceIp(name, namespace, retry = 20000) {
    let externalIp = "";
    try {
      log.info(`Fetching current service state for service ${name} in ${namespace}`);
      ...
      if (
        ...
        log.warn(`Service ${name} not ready, retry in ${retry / 1000} seconds `);
        await sleep(retry);
      ...
      if (err.response && err.body && err.body.code === 403) {
        log.warn("It seems that the service account doesn't have the permissions to view services");
