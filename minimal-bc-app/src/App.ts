...
export const createHashFromData = data => crypto.createHash('sha256').update(data).digest('hex');
...
router.post("/streams/:streamName/items", async (req, res) => {
  ...
  const response = await multichain(Publish(streamId, key, createHashFromData(value)))
  res.send(response.result)
})
